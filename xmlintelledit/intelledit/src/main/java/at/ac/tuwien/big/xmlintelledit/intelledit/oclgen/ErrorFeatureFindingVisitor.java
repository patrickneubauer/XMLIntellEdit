...
public class ErrorFeatureFindingVisitor implements Visitor<String, EClassifier, EOperation, EStructuralFeature, EEnumLiteral, EParameter, EObject, CallOperationAction, SendSignalAction, Constraint> {
	
	private final EClass context;
	private final Map<String, String> variables = new HashMap<>();
	private final Set<EStructuralFeature> features = new HashSet<EStructuralFeature>();
	
	public ErrorFeatureFindingVisitor(EClass context) {
	...
	public static String findFeature(EClass context, String expression) {
		Set<EStructuralFeature> features = findFeatures(context, expression);
		if (features.size() == 1) {
			return "self.eClass().getEStructuralFeature(\"" + features.iterator().next().getName() + "\")";
		} else {
			return "null";
		}
	}
	
	public static Set<EStructuralFeature> findFeatures(EClass context, String expression) {
		OCL ocl = OCL.newInstance();
		Helper oclHelper = ocl.createOCLHelper();

		oclHelper.setContext(context);
		ErrorFeatureFindingVisitor visitor = new ErrorFeatureFindingVisitor(context);
	...
	public String visitPropertyCallExp(PropertyCallExp<EClassifier, EStructuralFeature> callExp) {
		...
		if (context.isSuperTypeOf(callExp.getReferredProperty().getEContainingClass())) {
			features.add(callExp.getReferredProperty());
		}
		return null;
	}
