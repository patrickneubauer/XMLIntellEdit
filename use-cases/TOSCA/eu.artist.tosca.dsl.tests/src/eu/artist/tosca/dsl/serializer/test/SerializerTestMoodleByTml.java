...
/**
 * In case of TML we want the following: TML --> XMI --> XML (or directly: TML
 * --> XML) and reverse (i.e., back to the TML representation) XML --> XMI
 * --> TML (or directly: XML --> TML)
 * 
...
public class SerializerTestMoodleByTml {
	...
	@Test
	public void _4_xmiToTmlTest() throws IOException {
		...
		URI outputModelResourceURI = URI.createFileURI(new File(OUTPUT_MODEL_PATH + INPUT_MODEL.substring(INPUT_MODEL.lastIndexOf('/'), INPUT_MODEL.lastIndexOf('.')).concat("_reverse.tml")).getAbsolutePath());
		TMLSerializer serializer = new TMLSerializer();
		serializer.writeOutput(inputModelResourceURI, outputModelResourceURI, XMLResource.OPTION_EXTENDED_META_DATA);
	}// xmiToTmlTest

	@Test
	public void _5_tmlToXmlDirectlyTest() throws IOException {
		URI inputModelResourceURI = URI.createFileURI(new File(OUTPUT_MODEL_PATH + INPUT_MODEL.substring(INPUT_MODEL.lastIndexOf('/'), INPUT_MODEL.lastIndexOf('.')).concat("_reverse.tml")).getAbsolutePath());
	...
	@Test
	public void _6_xmlToTmlDirectlyTest() throws IOException {
		URI inputModelResourceURI = URI.createFileURI(new File(OUTPUT_MODEL_PATH + INPUT_MODEL.substring(INPUT_MODEL.lastIndexOf('/'), INPUT_MODEL.lastIndexOf('.')).concat(".xml")).getAbsolutePath());
